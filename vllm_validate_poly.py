...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run validation on model activations and commits."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Directory to save outputs and commits.",
    )
    parser.add_argument(
        "--decode_model_name",
        type=str,
        required=True,
        help="Model name used for decoding.",
    )
    parser.add_argument(
        "--validate_model_name",
        type=str,
        required=True,
        help="Model name used for validation.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="Maximum number of tokens for decoding.",
    )
    parser.add_argument(
        "--decode_batching_size", type=int, default=32, help="Batch size for decoding."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--scale_decode_mantissa", type=str, default="no", help="Scale decode mantissa."
    )
...
def check(
    activations: list[torch.Tensor], proof: list[str]
    ...
    for act, b_poly in zip(activations, proof):
        ...
        decode_topk_values = torch.tensor(
            [poly(i) for i in prefill_topk_indices], dtype=torch.uint16
        ...
        for i, index in enumerate(prefill_topk_indices):
            if index in dec_i_2_topk_i:
                ...
                if decode_exp[dec_i_2_topk_i[index]] == prefill_exp[i]:
                    exp_intersection += 1
                    mant_errs.append(
                        abs(decode_mants[dec_i_2_topk_i[index]] - prefill_mants[i])
                    )
    ...
    for mant_err_mean, mant_err_median, exp_intersection in zip(
        mant_err_means, mant_err_medians, exp_intersections
    ):
        if (
            mant_err_mean > TMEAN
            or mant_err_median > TMEDIAN
            or exp_intersection < TEXP
        ):
            print(
                f"VERIFICATION FAILED: Mantissa error mean: {mant_err_mean} above {TMEAN} or median: {mant_err_median} above {TMEDIAN} or exp intersections: {exp_intersection} below {TEXP}"
            )
        else:
            print(
                f"VERIFICATION PASSED: Mantissa error mean: {mant_err_means} below {TMEAN} and median: {mant_err_medians} below {TMEDIAN} and exp intersections: {exp_intersections} above {TEXP}"
            )

    return topk_intersections, exp_intersections, mant_err_means, mant_err_medians


def segment_prefill_activations(
    activations: torch.Tensor, max_decode_tokens: int, decode_batching_size: int
) -> list[torch.Tensor]:
    ret: list[torch.Tensor] = [activations[:, :-max_decode_tokens]]
    for i in range(
        activations.size(1) - max_decode_tokens,
        activations.size(1),
...
def main(args):
    ...
    with open(
        save_dir / f"poly_{args.decode_model_name.replace('/', '--')}_128.bin", "rb"
    ) as f:
        polys = [
            [
                f.read(258)
                for _j in range(1 + args.max_decode_tokens // args.decode_batching_size)
            ]
    ...
    model.model.norm.register_forward_hook(activation_saving_hook)
    ...
    for i, input_ids in tqdm(enumerate(outputs), total=len(outputs)):
        ...
        _ = llm.generate(
            tokens_prompt,
            SamplingParams(temperature=0.8, top_p=0.95, ignore_eos=True, max_tokens=1),
        )
        activations = segment_prefill_activations(
            saved_activations[0].unsqueeze(0),
            args.max_decode_tokens,
            args.decode_batching_size,
        )
    ...
    output_file = (
        save_dir
        / f"poly_validation_{args.validate_model_name.replace('/', '--')}_{args.attn}_{args.dtype}_{args.tp}A100_on_{args.decode_model_name.replace('/', '--')}.parquet"
    )

...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run activation saving and inference generation with a language model."
    )
    ...
    parser.add_argument(
        "--n_samples", type=int, default=4, help="Number of samples to generate."
    )
    parser.add_argument(
        "--save_dir", type=str, default="just4", help="Directory to save outputs."
    )
    parser.add_argument(
        "--max_decode_tokens",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Batching size for decoding.",
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="None",
        help="System prompt to prepend to each input.",
    )
...
def build_activation_commit(
    activations: list[torch.Tensor], decode_batching_size: int
) -> list[str]:
    commits = []
...
def main(args):
    ...
    sampling_params = SamplingParams(
        ...
        top_p=0.95,
        ignore_eos=True,
        max_tokens=args.max_decode_tokens + 1,
    )
    llm = LLM(
    ...
    for prompt in tqdm(prompts):
        ...
        act_commit = build_activation_commit(
            saved_activations, args.decode_batching_size
        )
